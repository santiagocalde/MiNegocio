            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
              <div style={{ position: 'relative' }}>
                <select value={formData.prefijo} onChange={e => setFormData({ ...formData, prefijo: e.target.value })} style={{ width: 110, padding: '16px 12px', background: 'var(--lp-paper-sunken)', border: '1px solid var(--lp-line-strong)', borderRadius: 12, color: 'var(--lp-ink)', fontSize: '1.1rem', fontWeight: 600, outline: 'none', appearance: 'none', cursor: 'pointer', transition: 'all 0.2s' }} onFocus={e => e.target.style.borderColor = 'var(--lp-primary)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'}>
                  {prefijos.map(p => (
                    <option key={p.code} value={p.code} style={{ background: '#1E3A5F', color: 'var(--lp-ink)' }}>{p.country} {p.code}</option>
                  ))}
                </select>
  ...
  return (
    <div className="lp-noise" style={{ fontFamily: 'var(--lp-font-body)', color: 'var(--lp-text)', minHeight: '100vh', position: 'relative', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ padding: 'clamp(16px, 4vw, 32px) clamp(16px, 4vw, 40px)', position: 'relative', zIndex: 10, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <button onClick={() => navigate('/')} style={{ position: 'absolute', left: 'clamp(16px, 4vw, 40px)', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--lp-line-strong)', color: 'var(--lp-ink)', padding: '10px 16px', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.9rem', fontWeight: 600 }}>
          <Svg.ArrowLeft /> Salir
        </button>
      </nav>
