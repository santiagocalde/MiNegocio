  ...
  return (
    <div style={{ flex: 1, background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-color)' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', margin: 0 }}>Total: ${displayTotal.toLocaleString('es-AR')}</p>
      </div>
            ...
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                    <span style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: '0.9rem' }}>$ {item.price.toLocaleString('es-AR')}</span>
                    <span style={{ background: 'rgba(239, 68, 68, 0.2)', border: '1px solid rgba(239, 68, 68, 0.3)', color: '#FCA5A5', padding: '2px 4px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 800 }}>Stock: {item.stock} u</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text-primary)' }}>${(item.price * item.qty).toLocaleString('es-AR')}</div>
                </div>
