...
async def admin_delete_business(
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            ...
            await conn.execute("DELETE FROM categories WHERE business_id = $1", business_id)
            ...
            await conn.execute("DELETE FROM sucursales WHERE business_id = $1", business_id)
            await conn.execute("DELETE FROM payment_events WHERE business_id = $1", business_id)
            await conn.execute("DELETE FROM payment_intents WHERE business_id = $1", business_id)
